<template>
  ...
  <button @click="fullNavOpen = !fullNavOpen" aria-label="Voir le menu" class="md:hidden group h-12 w-12 px-2 pt-2.5 fixed backdrop-blur-sm rounded-lg right-6 top-6 flex flex-col gap-2 z-40">
    <span class="w-8 bg-black dark:bg-white h-1 block rounded-full drop-shadow-lg"></span>
    <span class="group-hover:w-1/3 w-8 duration-150 bg-black dark:bg-white h-1 block rounded-full drop-shadow-lg"></span>
    <span class="w-8 bg-black dark:bg-white h-1 block rounded-full drop-shadow-lg"></span>
  </button>
