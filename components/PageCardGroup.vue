...
export default {
  ...
  mounted() {
    ...
    if (this.elementsCategory === 'toVisit' && localStorage.elementsMinimizedAVisiter)
      this.elementsMinimized = localStorage.elementsMinimizedAVisiter === 'true'
    if (this.elementsCategory === 'restore' && localStorage.elementsMinimizedSeRestaurer)
      this.elementsMinimized = localStorage.elementsMinimizedSeRestaurer === 'true'
    this.updateElementList()
  },
  methods: {
    ...
    GetFilter() {
      ...
      this.propElementsList.forEach((element) => {
        element.subCategory.forEach((sous_category_elem) => {
          !filters.includes(sous_category_elem) && filters.push(sous_category_elem)
        })
      })
      ...
      this.propElementsList.forEach((element) => {
        element.subCategory.forEach((sous_category_elem) => {
          if (sous_category_elem === newVisibleCategory)
            !this.filteredElementsList.includes(sous_category_elem) && this.filteredElementsList.push(element)
        })
      })
      this.updateElementList()
    },
    ...
    <div class="grid gap-6">
      ...
      >
        <PageCard
          ...
          :short-desc="element.category ? $t(`${targetPage}.${element.category}.${element.i18n}.shortDesc`) : $t(`${targetPage}.${element.i18n}.shortDesc`)"
